...
import com.wrbug.polymarketbot.entity.CryptoTailScalpStopConfig
...
class CryptoTailStrategyService(
    ...
    fun create(request: CryptoTailStrategyCreateRequest): Result<CryptoTailStrategyDto> {
        return try {
            ...
            val entity = CryptoTailStrategy(
                ...
                tailDiffOddsLagStrongEdgeBypass = td.oddsLagStrongEdgeBypass,
                ...
                scalpEntryConfig = CryptoTailScalpEntryConfig(
                ...
                scalpStopConfig = CryptoTailScalpStopConfig(
                ...
                scalpSpotLeadConfig = CryptoTailScalpSpotLeadConfig(
                    enabled = sc.spotLeadEnabled,
                    source = sc.spotLeadSource,
                    maxAgeMs = sc.spotLeadMaxAgeMs,
                    flipDistanceSigma = sc.spotLeadFlipDistanceSigma,
    ...
    private data class TailDiffResolved(
        ...
        val oddsLagStrongEdgeBypass: Boolean,
    ...
    private fun resolveTailDiffCreate(r: CryptoTailStrategyCreateRequest): TailDiffResolved = TailDiffResolved(
        ...
        oddsLagStrongEdgeBypass = r.tailDiffOddsLagStrongEdgeBypass ?: false,
    ...
    private fun resolveTailDiffUpdate(r: CryptoTailStrategyUpdateRequest, e: CryptoTailStrategy): TailDiffResolved = TailDiffResolved(
        ...
        oddsLagStrongEdgeBypass = r.tailDiffOddsLagStrongEdgeBypass ?: e.tailDiffOddsLagStrongEdgeBypass,
    ...
    private fun resolveScalpCreate(r: CryptoTailStrategyCreateRequest): ScalpResolved = ScalpResolved(
        entryMinPrice = r.scalp?.scalpEntryMinPrice?.toSafeBigDecimal() ?: BigDecimal("0.96"),
        entryMaxPrice = r.scalp?.scalpEntryMaxPrice?.toSafeBigDecimal() ?: BigDecimal("0.97"),
        maxFillPrice = r.scalp?.scalpMaxFillPrice?.toSafeBigDecimal() ?: BigDecimal("0.975"),
        windowStartSeconds = r.scalp?.scalpWindowStartSeconds ?: 0,
        windowEndSeconds = r.scalp?.scalpWindowEndSeconds ?: 0,
        minRemainingSeconds = r.scalp?.scalpMinRemainingSeconds ?: 30,
        minExitBidDepthUsdc = r.scalp?.scalpMinExitBidDepthUsdc?.takeIf { it.isNotBlank() }?.toSafeBigDecimal(),
        reversalGateEnabled = r.scalp?.scalpReversalGateEnabled ?: true,
        minModelProb = r.scalp?.scalpMinModelProb?.toSafeBigDecimal() ?: BigDecimal("0.95"),
        minEdge = r.scalp?.scalpMinEdge?.toSafeBigDecimal() ?: BigDecimal.ZERO,
        statsSource = normalizeScalpStatsSource(r.scalp?.scalpStatsSource),
        statsLookbackDays = r.scalp?.scalpStatsLookbackDays ?: 180,
        statsMinSamples = r.scalp?.scalpStatsMinSamples ?: 30,
        requireStats = r.scalp?.scalpRequireStats ?: false,
        maxConcurrentSameDirection = r.scalp?.scalpMaxConcurrentSameDirection?.takeIf { it > 0 },
        holdWinnerToSettle = r.scalp?.scalpHoldWinnerToSettle ?: true,
        tpPrice = r.scalp?.scalpTpPrice?.toSafeBigDecimal() ?: BigDecimal("0.99"),
        stopEnabled = r.scalp?.scalpStopEnabled ?: true,
        stopOffset = r.scalp?.scalpStopOffset?.toSafeBigDecimal() ?: BigDecimal("0.05"),
        stopMinPrice = r.scalp?.scalpStopMinPrice?.toSafeBigDecimal() ?: BigDecimal("0.90"),
        minOddsAfterEntry = r.scalp?.scalpMinOddsAfterEntry?.toSafeBigDecimal() ?: BigDecimal("0.93"),
        underlyingStopEnabled = r.scalp?.scalpUnderlyingStopEnabled ?: true,
        underlyingStopSigma = r.scalp?.scalpUnderlyingStopSigma?.toSafeBigDecimal() ?: BigDecimal("0.30"),
        reverseVelocityStopEnabled = r.scalp?.scalpReverseVelocityStopEnabled ?: true,
        maxReverseVelocitySigma = r.scalp?.scalpMaxReverseVelocitySigma?.toSafeBigDecimal() ?: BigDecimal("0.40"),
        reverseVelocityWindowSeconds = r.scalp?.scalpReverseVelocityWindowSeconds ?: 10,
        minModelProbAfterEntry = r.scalp?.scalpMinModelProbAfterEntry?.toSafeBigDecimal() ?: BigDecimal("0.80"),
        maxDiffRetracePct = r.scalp?.scalpMaxDiffRetracePct?.toSafeBigDecimal() ?: BigDecimal("0.35"),
        catastropheBidFloor = r.scalp?.scalpCatastropheBidFloor?.toSafeBigDecimal() ?: BigDecimal("0.88"),
        catastropheImmediate = r.scalp?.scalpCatastropheImmediate ?: true,
        catastropheFloorRatio = r.scalp?.scalpCatastropheFloorRatio?.toSafeBigDecimal() ?: BigDecimal("0.85"),
        wsFreshnessSkipRestMs = r.scalp?.scalpWsFreshnessSkipRestMs ?: 500,
        entryRequoteMax = r.scalp?.scalpEntryRequoteMax ?: 2,
        requireUnderlyingAgreement = r.scalp?.scalpRequireUnderlyingAgreement ?: true,
        entryMinPwin = r.scalp?.scalpEntryMinPwin?.toSafeBigDecimal() ?: BigDecimal("0.90"),
        smartStopMinPwin = r.scalp?.scalpSmartStopMinPwin?.toSafeBigDecimal() ?: BigDecimal("0.70"),
        smartStopMinSafeRatio = r.scalp?.scalpSmartStopMinSafeRatio?.toSafeBigDecimal() ?: BigDecimal("1.30"),
        hardFloorRatio = r.scalp?.scalpHardFloorRatio?.toSafeBigDecimal() ?: BigDecimal("0.50"),
        dailyLossLimitUsdc = r.scalp?.scalpDailyLossLimitUsdc?.takeIf { it.isNotBlank() }?.toSafeBigDecimal(),
        consecLossPauseCount = r.scalp?.scalpConsecLossPauseCount ?: 0,
        consecLossStopCount = r.scalp?.scalpConsecLossStopCount ?: 3,
        gapGateEnabled = r.scalp?.scalpGapGateEnabled ?: false,
        minEntryDiffSigma = r.scalp?.scalpMinEntryDiffSigma?.toSafeBigDecimal() ?: BigDecimal.ZERO,
        minEntryGapAbs = r.scalp?.scalpMinEntryGapAbs?.toSafeBigDecimal() ?: BigDecimal.ZERO,
        gapGateRemainingLo = r.scalp?.scalpGapGateRemainingLo ?: 0,
        gapGateRemainingHi = r.scalp?.scalpGapGateRemainingHi ?: 0,
        evLimitMode = normalizeScalpEvLimitMode(r.scalp?.scalpEvLimitMode),
        evGuardMargin = r.scalp?.scalpEvGuardMargin?.toSafeBigDecimal() ?: BigDecimal("0.10"),
        lateStopEnabled = r.scalp?.scalpLateStopEnabled ?: false,
        lateStopSeconds = r.scalp?.scalpLateStopSeconds ?: 15,
        latePeakDrawdown = r.scalp?.scalpLatePeakDrawdown?.toSafeBigDecimal() ?: BigDecimal("0.18"),
        lateBidFloor = r.scalp?.scalpLateBidFloor?.toSafeBigDecimal() ?: BigDecimal("0.70"),
        disableWickGuardOnLateStop = r.scalp?.scalpDisableWickGuardOnLateStop ?: true,
        lateStopRequireWeakModel = r.scalp?.scalpLateStopRequireWeakModel ?: false,
        lateFastPollSeconds = r.scalp?.scalpLateFastPollSeconds ?: 0,
        lateFastPollMs = r.scalp?.scalpLateFastPollMs ?: 300,
        emergencyRetryCount = r.scalp?.scalpEmergencyRetryCount ?: 0,
        emergencyRetryIntervalMs = r.scalp?.scalpEmergencyRetryIntervalMs ?: 150,
        lateIgnoreWorstPriceSeconds = r.scalp?.scalpLateIgnoreWorstPriceSeconds ?: 0,
        lateScaleOutSeconds = r.scalp?.scalpLateScaleOutSeconds ?: 0,
        lateScaleOutRatio = r.scalp?.scalpLateScaleOutRatio?.toSafeBigDecimal() ?: BigDecimal.ZERO,
        spotLeadEnabled = r.scalp?.scalpSpotLeadEnabled ?: false,
        spotLeadSource = normalizeScalpSpotLeadSource(r.scalp?.scalpSpotLeadSource),
        spotLeadMaxAgeMs = r.scalp?.scalpSpotLeadMaxAgeMs ?: 3000,
        spotLeadFlipDistanceSigma = r.scalp?.scalpSpotLeadFlipDistanceSigma?.toSafeBigDecimal() ?: BigDecimal.ZERO,
    ...
    private fun resolveScalpUpdate(r: CryptoTailStrategyUpdateRequest, e: CryptoTailStrategy): ScalpResolved = ScalpResolved(
        entryMinPrice = r.scalp?.scalpEntryMinPrice?.toSafeBigDecimal() ?: e.scalpEntryMinPrice,
        entryMaxPrice = r.scalp?.scalpEntryMaxPrice?.toSafeBigDecimal() ?: e.scalpEntryMaxPrice,
        maxFillPrice = r.scalp?.scalpMaxFillPrice?.toSafeBigDecimal() ?: e.scalpMaxFillPrice,
        windowStartSeconds = r.scalp?.scalpWindowStartSeconds ?: e.scalpWindowStartSeconds,
        windowEndSeconds = r.scalp?.scalpWindowEndSeconds ?: e.scalpWindowEndSeconds,
        minRemainingSeconds = r.scalp?.scalpMinRemainingSeconds ?: e.scalpMinRemainingSeconds,
        minExitBidDepthUsdc = resolveNullableBigDecimalUpdate(r.scalp?.scalpMinExitBidDepthUsdc, e.scalpMinExitBidDepthUsdc),
        reversalGateEnabled = r.scalp?.scalpReversalGateEnabled ?: e.scalpReversalGateEnabled,
        minModelProb = r.scalp?.scalpMinModelProb?.toSafeBigDecimal() ?: e.scalpMinModelProb,
        minEdge = r.scalp?.scalpMinEdge?.toSafeBigDecimal() ?: e.scalpMinEdge,
        statsSource = r.scalp?.scalpStatsSource?.let { normalizeScalpStatsSource(it) } ?: e.scalpStatsSource,
        statsLookbackDays = r.scalp?.scalpStatsLookbackDays ?: e.scalpStatsLookbackDays,
        statsMinSamples = r.scalp?.scalpStatsMinSamples ?: e.scalpStatsMinSamples,
        requireStats = r.scalp?.scalpRequireStats ?: e.scalpRequireStats,
        maxConcurrentSameDirection = resolveNullableIntUpdate(r.scalp?.scalpMaxConcurrentSameDirection, e.scalpMaxConcurrentSameDirection),
        holdWinnerToSettle = r.scalp?.scalpHoldWinnerToSettle ?: e.scalpHoldWinnerToSettle,
        tpPrice = r.scalp?.scalpTpPrice?.toSafeBigDecimal() ?: e.scalpTpPrice,
        stopEnabled = r.scalp?.scalpStopEnabled ?: e.scalpStopEnabled,
        stopOffset = r.scalp?.scalpStopOffset?.toSafeBigDecimal() ?: e.scalpStopOffset,
        stopMinPrice = r.scalp?.scalpStopMinPrice?.toSafeBigDecimal() ?: e.scalpStopMinPrice,
        minOddsAfterEntry = r.scalp?.scalpMinOddsAfterEntry?.toSafeBigDecimal() ?: e.scalpMinOddsAfterEntry,
        underlyingStopEnabled = r.scalp?.scalpUnderlyingStopEnabled ?: e.scalpUnderlyingStopEnabled,
        underlyingStopSigma = r.scalp?.scalpUnderlyingStopSigma?.toSafeBigDecimal() ?: e.scalpUnderlyingStopSigma,
        reverseVelocityStopEnabled = r.scalp?.scalpReverseVelocityStopEnabled ?: e.scalpReverseVelocityStopEnabled,
        maxReverseVelocitySigma = r.scalp?.scalpMaxReverseVelocitySigma?.toSafeBigDecimal() ?: e.scalpMaxReverseVelocitySigma,
        reverseVelocityWindowSeconds = r.scalp?.scalpReverseVelocityWindowSeconds ?: e.scalpReverseVelocityWindowSeconds,
        minModelProbAfterEntry = r.scalp?.scalpMinModelProbAfterEntry?.toSafeBigDecimal() ?: e.scalpMinModelProbAfterEntry,
        maxDiffRetracePct = r.scalp?.scalpMaxDiffRetracePct?.toSafeBigDecimal() ?: e.scalpMaxDiffRetracePct,
        catastropheBidFloor = r.scalp?.scalpCatastropheBidFloor?.toSafeBigDecimal() ?: e.scalpCatastropheBidFloor,
        catastropheImmediate = r.scalp?.scalpCatastropheImmediate ?: e.scalpCatastropheImmediate,
        catastropheFloorRatio = r.scalp?.scalpCatastropheFloorRatio?.toSafeBigDecimal() ?: e.scalpCatastropheFloorRatio,
        wsFreshnessSkipRestMs = r.scalp?.scalpWsFreshnessSkipRestMs ?: e.scalpWsFreshnessSkipRestMs,
        entryRequoteMax = r.scalp?.scalpEntryRequoteMax ?: e.scalpEntryRequoteMax,
        requireUnderlyingAgreement = r.scalp?.scalpRequireUnderlyingAgreement ?: e.scalpRequireUnderlyingAgreement,
        entryMinPwin = r.scalp?.scalpEntryMinPwin?.toSafeBigDecimal() ?: e.scalpEntryMinPwin,
        smartStopMinPwin = r.scalp?.scalpSmartStopMinPwin?.toSafeBigDecimal() ?: e.scalpSmartStopMinPwin,
        smartStopMinSafeRatio = r.scalp?.scalpSmartStopMinSafeRatio?.toSafeBigDecimal() ?: e.scalpSmartStopMinSafeRatio,
        hardFloorRatio = r.scalp?.scalpHardFloorRatio?.toSafeBigDecimal() ?: e.scalpHardFloorRatio,
        dailyLossLimitUsdc = resolveNullableBigDecimalUpdate(r.scalp?.scalpDailyLossLimitUsdc, e.scalpDailyLossLimitUsdc),
        consecLossPauseCount = r.scalp?.scalpConsecLossPauseCount ?: e.scalpConsecLossPauseCount,
        consecLossStopCount = r.scalp?.scalpConsecLossStopCount ?: e.scalpConsecLossStopCount,
        gapGateEnabled = r.scalp?.scalpGapGateEnabled ?: e.scalpGapGateEnabled,
        minEntryDiffSigma = r.scalp?.scalpMinEntryDiffSigma?.toSafeBigDecimal() ?: e.scalpMinEntryDiffSigma,
        minEntryGapAbs = r.scalp?.scalpMinEntryGapAbs?.toSafeBigDecimal() ?: e.scalpMinEntryGapAbs,
        gapGateRemainingLo = r.scalp?.scalpGapGateRemainingLo ?: e.scalpGapGateRemainingLo,
        gapGateRemainingHi = r.scalp?.scalpGapGateRemainingHi ?: e.scalpGapGateRemainingHi,
        evLimitMode = r.scalp?.scalpEvLimitMode?.let { normalizeScalpEvLimitMode(it) } ?: e.scalpEvLimitMode,
        evGuardMargin = r.scalp?.scalpEvGuardMargin?.toSafeBigDecimal() ?: e.scalpEvGuardMargin,
        lateStopEnabled = r.scalp?.scalpLateStopEnabled ?: e.scalpLateStopEnabled,
        lateStopSeconds = r.scalp?.scalpLateStopSeconds ?: e.scalpLateStopSeconds,
        latePeakDrawdown = r.scalp?.scalpLatePeakDrawdown?.toSafeBigDecimal() ?: e.scalpLatePeakDrawdown,
        lateBidFloor = r.scalp?.scalpLateBidFloor?.toSafeBigDecimal() ?: e.scalpLateBidFloor,
        disableWickGuardOnLateStop = r.scalp?.scalpDisableWickGuardOnLateStop ?: e.scalpDisableWickGuardOnLateStop,
        lateStopRequireWeakModel = r.scalp?.scalpLateStopRequireWeakModel ?: e.scalpLateStopRequireWeakModel,
        lateFastPollSeconds = r.scalp?.scalpLateFastPollSeconds ?: e.scalpLateFastPollSeconds,
        lateFastPollMs = r.scalp?.scalpLateFastPollMs ?: e.scalpLateFastPollMs,
        emergencyRetryCount = r.scalp?.scalpEmergencyRetryCount ?: e.scalpEmergencyRetryCount,
        emergencyRetryIntervalMs = r.scalp?.scalpEmergencyRetryIntervalMs ?: e.scalpEmergencyRetryIntervalMs,
        lateIgnoreWorstPriceSeconds = r.scalp?.scalpLateIgnoreWorstPriceSeconds ?: e.scalpLateIgnoreWorstPriceSeconds,
        lateScaleOutSeconds = r.scalp?.scalpLateScaleOutSeconds ?: e.scalpLateScaleOutSeconds,
        lateScaleOutRatio = r.scalp?.scalpLateScaleOutRatio?.toSafeBigDecimal() ?: e.scalpLateScaleOutRatio,
        spotLeadEnabled = r.scalp?.scalpSpotLeadEnabled ?: e.scalpSpotLeadEnabled,
        spotLeadSource = r.scalp?.scalpSpotLeadSource?.let { normalizeScalpSpotLeadSource(it) } ?: e.scalpSpotLeadSource,
        spotLeadMaxAgeMs = r.scalp?.scalpSpotLeadMaxAgeMs ?: e.scalpSpotLeadMaxAgeMs,
        spotLeadFlipDistanceSigma = r.scalp?.scalpSpotLeadFlipDistanceSigma?.toSafeBigDecimal() ?: e.scalpSpotLeadFlipDistanceSigma,
    ...
    private fun entityToDto(e: CryptoTailStrategy, lastTriggerAt: Long?): CryptoTailStrategyDto {
        ...
        return CryptoTailStrategyDto(
            ...
            tailDiffOddsLagStrongEdgeBypass = e.tailDiffOddsLagStrongEdgeBypass,
            ...
            scalp = CryptoTailStrategyScalpDto(
