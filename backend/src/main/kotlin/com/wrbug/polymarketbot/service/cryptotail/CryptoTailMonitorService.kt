...
class CryptoTailMonitorService(
    ...
    private fun buildPushData(strategy: CryptoTailStrategy, priceData: StrategyPriceData): CryptoTailMonitorPushData {
        ...
        return CryptoTailMonitorPushData(
            ...
            trailingStartLine = entryFill?.add(strategy.trailingStartDelta)?.setScale(4, RoundingMode.HALF_UP)?.toPlainString(),
            trailingStopLine = peakBid?.subtract(strategy.trailingDrawdown)?.setScale(4, RoundingMode.HALF_UP)?.toPlainString(),
            ...
            wickClosePosition = if (wick.available) wick.closePosition.toPlainString() else null,
            wickRangeSigmaRatio = if (wick.available) wick.rangeSigmaRatio.toPlainString() else null,
            wickTickCount = if (wick.available) wick.tickCount else null,
            wickQualityReason = if (wick.available) wick.qualityReason else null,
