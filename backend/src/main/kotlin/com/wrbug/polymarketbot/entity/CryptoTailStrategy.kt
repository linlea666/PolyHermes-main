...
data class CryptoTailStrategy(
    ...
    /** 进场与统计闸配置（V77+）：进场价区/窗口/反转率统计/EV 钳价/并发/退出模式。字段定义见 CryptoTailScalpEntryConfig。 */
    @Embedded
    val scalpEntryConfig: CryptoTailScalpEntryConfig = CryptoTailScalpEntryConfig(),

    /** 止盈/止损/熔断核心配置。字段定义见 CryptoTailScalpStopConfig。 */
    @Embedded
    val scalpStopConfig: CryptoTailScalpStopConfig = CryptoTailScalpStopConfig(),

    /** 执行与智能止损配置（WS 新鲜度/重报价/方向确认/智能硬止损旁路/无条件深底线）。字段定义见 CryptoTailScalpExecConfig。 */
    @Embedded
    val scalpExecConfig: CryptoTailScalpExecConfig = CryptoTailScalpExecConfig(),

    /** 风控配置（当日亏损熔断/连续亏损/进场价差闸）。字段定义见 CryptoTailScalpRiskConfig。 */
    @Embedded
    val scalpRiskConfig: CryptoTailScalpRiskConfig = CryptoTailScalpRiskConfig(),

    /** 尾盘韧性退出配置（V91/V92：尾盘止损/提速/紧急重试/忽略地板/主动减仓）。字段定义见 CryptoTailScalpLateExitConfig。 */
    @Embedded
    val scalpLateExitConfig: CryptoTailScalpLateExitConfig = CryptoTailScalpLateExitConfig(),

    /** 现货价"领先早警"配置（V93）。字段定义见 CryptoTailScalpSpotLeadConfig。 */
    @Embedded
    ...
    /**
     * 以下为各 SCALP @Embedded 配置值对象的"同名直通属性"（委托对应 config）。
     * 拆分后字段已迁入 @Embeddable 值对象，这里保持与历史扁平字段完全一致的读取 API，
     * 确保决策/日志/DTO 映射等全部调用点零改动。均为计算属性（无 backing field），
     * 实体使用字段访问策略，Hibernate 不会将其视为持久化列。
     */
    // --- scalpEntryConfig（进场与统计闸）---
    val scalpEntryMinPrice: BigDecimal get() = scalpEntryConfig.scalpEntryMinPrice
    val scalpEntryMaxPrice: BigDecimal get() = scalpEntryConfig.scalpEntryMaxPrice
    val scalpMaxFillPrice: BigDecimal get() = scalpEntryConfig.scalpMaxFillPrice
    val scalpEvLimitMode: String get() = scalpEntryConfig.scalpEvLimitMode
    val scalpEvGuardMargin: BigDecimal get() = scalpEntryConfig.scalpEvGuardMargin
    val scalpWindowStartSeconds: Int get() = scalpEntryConfig.scalpWindowStartSeconds
    val scalpWindowEndSeconds: Int get() = scalpEntryConfig.scalpWindowEndSeconds
    val scalpMinRemainingSeconds: Int get() = scalpEntryConfig.scalpMinRemainingSeconds
    val scalpMinExitBidDepthUsdc: BigDecimal? get() = scalpEntryConfig.scalpMinExitBidDepthUsdc
    val scalpReversalGateEnabled: Boolean get() = scalpEntryConfig.scalpReversalGateEnabled
    val scalpMinModelProb: BigDecimal get() = scalpEntryConfig.scalpMinModelProb
    val scalpMinEdge: BigDecimal get() = scalpEntryConfig.scalpMinEdge
    val scalpStatsSource: String get() = scalpEntryConfig.scalpStatsSource
    val scalpStatsLookbackDays: Int get() = scalpEntryConfig.scalpStatsLookbackDays
    val scalpStatsMinSamples: Int get() = scalpEntryConfig.scalpStatsMinSamples
    val scalpRequireStats: Boolean get() = scalpEntryConfig.scalpRequireStats
    val scalpMaxConcurrentSameDirection: Int? get() = scalpEntryConfig.scalpMaxConcurrentSameDirection
    val scalpHoldWinnerToSettle: Boolean get() = scalpEntryConfig.scalpHoldWinnerToSettle

    // --- scalpStopConfig（止盈/止损/熔断）---
    val scalpTpPrice: BigDecimal get() = scalpStopConfig.scalpTpPrice
    val scalpStopEnabled: Boolean get() = scalpStopConfig.scalpStopEnabled
    val scalpStopOffset: BigDecimal get() = scalpStopConfig.scalpStopOffset
    val scalpStopMinPrice: BigDecimal get() = scalpStopConfig.scalpStopMinPrice
    val scalpMinOddsAfterEntry: BigDecimal get() = scalpStopConfig.scalpMinOddsAfterEntry
    val scalpUnderlyingStopEnabled: Boolean get() = scalpStopConfig.scalpUnderlyingStopEnabled
    val scalpUnderlyingStopSigma: BigDecimal get() = scalpStopConfig.scalpUnderlyingStopSigma
    val scalpReverseVelocityStopEnabled: Boolean get() = scalpStopConfig.scalpReverseVelocityStopEnabled
    val scalpMaxReverseVelocitySigma: BigDecimal get() = scalpStopConfig.scalpMaxReverseVelocitySigma
    val scalpReverseVelocityWindowSeconds: Int get() = scalpStopConfig.scalpReverseVelocityWindowSeconds
    val scalpMinModelProbAfterEntry: BigDecimal get() = scalpStopConfig.scalpMinModelProbAfterEntry
    val scalpMaxDiffRetracePct: BigDecimal get() = scalpStopConfig.scalpMaxDiffRetracePct
    ...
    val scalpEntryRequoteMax: Int get() = scalpExecConfig.scalpEntryRequoteMax
    val scalpRequireUnderlyingAgreement: Boolean get() = scalpExecConfig.scalpRequireUnderlyingAgreement
    val scalpEntryMinPwin: BigDecimal get() = scalpExecConfig.scalpEntryMinPwin
    val scalpSmartStopMinPwin: BigDecimal get() = scalpExecConfig.scalpSmartStopMinPwin
    val scalpSmartStopMinSafeRatio: BigDecimal get() = scalpExecConfig.scalpSmartStopMinSafeRatio
    val scalpHardFloorRatio: BigDecimal get() = scalpExecConfig.scalpHardFloorRatio

    // --- scalpRiskConfig（风控）---
    val scalpDailyLossLimitUsdc: BigDecimal? get() = scalpRiskConfig.scalpDailyLossLimitUsdc
    val scalpConsecLossPauseCount: Int get() = scalpRiskConfig.scalpConsecLossPauseCount
    val scalpConsecLossStopCount: Int get() = scalpRiskConfig.scalpConsecLossStopCount
    val scalpGapGateEnabled: Boolean get() = scalpRiskConfig.scalpGapGateEnabled
    val scalpMinEntryDiffSigma: BigDecimal get() = scalpRiskConfig.scalpMinEntryDiffSigma
    ...
    val scalpLateStopSeconds: Int get() = scalpLateExitConfig.scalpLateStopSeconds
    val scalpLatePeakDrawdown: BigDecimal get() = scalpLateExitConfig.scalpLatePeakDrawdown
    val scalpLateBidFloor: BigDecimal get() = scalpLateExitConfig.scalpLateBidFloor
    val scalpDisableWickGuardOnLateStop: Boolean get() = scalpLateExitConfig.scalpDisableWickGuardOnLateStop
    val scalpLateStopRequireWeakModel: Boolean get() = scalpLateExitConfig.scalpLateStopRequireWeakModel
    val scalpLateFastPollSeconds: Int get() = scalpLateExitConfig.scalpLateFastPollSeconds
    val scalpLateFastPollMs: Int get() = scalpLateExitConfig.scalpLateFastPollMs
    val scalpEmergencyRetryCount: Int get() = scalpLateExitConfig.scalpEmergencyRetryCount
    val scalpEmergencyRetryIntervalMs: Int get() = scalpLateExitConfig.scalpEmergencyRetryIntervalMs
    val scalpLateIgnoreWorstPriceSeconds: Int get() = scalpLateExitConfig.scalpLateIgnoreWorstPriceSeconds
    val scalpLateScaleOutSeconds: Int get() = scalpLateExitConfig.scalpLateScaleOutSeconds
    val scalpLateScaleOutRatio: BigDecimal get() = scalpLateExitConfig.scalpLateScaleOutRatio

    // --- scalpSpotLeadConfig（现货领先早警 V93）---
    val scalpSpotLeadEnabled: Boolean get() = scalpSpotLeadConfig.enabled
    val scalpSpotLeadSource: String get() = scalpSpotLeadConfig.source
    val scalpSpotLeadMaxAgeMs: Int get() = scalpSpotLeadConfig.maxAgeMs
    val scalpSpotLeadFlipDistanceSigma: BigDecimal get() = scalpSpotLeadConfig.flipDistanceSigma
    val scalpSpotLeadWickVetoEnabled: Boolean get() = scalpSpotLeadConfig.wickVetoEnabled
