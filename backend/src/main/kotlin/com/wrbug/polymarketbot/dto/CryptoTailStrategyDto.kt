...
/**
 * SCALP_FLIP 快进快出请求侧配置（V77/V91/V93）。
 * 独立为嵌套对象：父请求字段众多，若继续平铺会使构造器/合成方法参数槽
 * 触碰 JVM 方法描述符 255 槽硬上限（ClassFormatError）。字段均可空，
 * null 表示沿用默认值（创建）或保留原值（更新）。JSON 形如 {"scalp": {...}}。
 */
data class CryptoTailStrategyScalpRequest(
 ...
 *
 * 注：SCALP_FLIP 全部配置已收敛到嵌套对象 scalp(CryptoTailStrategyScalpRequest)，
 * 以避免本类构造器/合成方法参数槽触碰 JVM 255 槽硬上限（ClassFormatError）。
 */
data class CryptoTailStrategyCreateRequest(
    ...
    // ===== 快进快出模式 SCALP_FLIP（V77/V91/V93）收敛为嵌套对象，规避 JVM 255 参数槽上限 =====
    val scalp: CryptoTailStrategyScalpRequest? = null
)

/**
 * 加密价差策略更新请求
 *
 * 注：SCALP_FLIP 全部配置已收敛到嵌套对象 scalp(CryptoTailStrategyScalpRequest)，规避 JVM 255 参数槽上限。
 */
